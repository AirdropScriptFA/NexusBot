...
class NexusMiner {
  ...
  async start() {
    ...
    this.refreshDisplay();
    this.addLog(chalk.green('Account Initialized successfully'));
  }
  ...
  async startMining() {
    ...
    this.addLog(chalk.cyan('Activating Mining Process...'));
    let retries = 0;
    const maxRetries = 3;
        ...
        this.toggleCheckInterval = setInterval(async () => {
          try {
            ...
            if (toggleStatus.wasOff) {
              this.addLog(chalk.yellow('Mining Stopped, Try Reactivated Mining'));
              this.userInfo.status = 'Active';
              this.refreshDisplay();
            }
          } catch (error) {
  ...
  async stopMining() {
    ...
    this.addLog(chalk.cyan('Stopping Mining Process...'));
    try {
      const toggleStatus = await this.page.evaluate(() => {
        let toggle = document.querySelector('#connect-toggle-button');
        if (toggle) {
...
async function main() {
  ...
  screen.key(['escape', 'q', 'C-c'], async () => {
    for (const miner of miners) {
      ...
      if (miner.toggleCheckInterval) clearInterval(miner.toggleCheckInterval);
